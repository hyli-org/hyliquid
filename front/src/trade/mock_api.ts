import type { Fill, Instrument, Order, PerpPosition } from "./trade";
...
export async function fetchOrderbook(_symbol: string) {
...
export async function fetchOrders(): Promise<Order[]> {
    await delay(200 + Math.random() * 200);
    maybeFail(0.06);
    return [{ symbol: "ETH/PERP", side: "Ask", size: 1.0, type: "Limit", price: 2500, status: "Open" }];
}
