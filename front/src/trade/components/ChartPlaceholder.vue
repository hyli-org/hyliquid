...
function subscribeToCandlestick() {
    if (currentInstrument.value) {
        websocketManager.subscribeToCandlestick(currentInstrument.value.symbol, chartPreferences.intervalSeconds);
    }
}
...
function updateTimeScaleConfig() {
    ...
    let timeScaleConfig: any = {
        ...
        tickMarkFormatter: (time: number, _tickMarkType: any, locale: string) => {
            ...
            } else if (interval.seconds <= 86400) {
                // 1 day or less
    ...
    <div class="border-b border-[var(--border-default)] px-5 py-3">
        ...
        <div class="mb-3 flex items-center justify-between">
            <div class="flex items-center space-x-2">
                <span class="text-sm text-[var(--text-muted)]">Timeframe:</span>
                <div
                    class="flex items-center space-x-1 rounded-md border border-[var(--border-default)] bg-[var(--surface-input)] p-1"
                >
                    <button
                        v-for="interval in timeIntervals"
                        :key="interval.seconds"
                        @click="updateStepInterval(interval)"
                        :class="[
                            'rounded px-2 py-1 text-xs transition-colors',
                            isIntervalSelected(interval)
                                ? 'bg-[var(--accent-soft)] text-[var(--text-accent)] shadow-sm'
                                : 'text-[var(--text-secondary)] hover:bg-[var(--surface-header)] hover:text-[var(--text-primary)]',
                        ]"
                    >
                        {{ interval.label }}
                    </button>
            ...
            <!-- Chart Info -->
            <div class="text-xs text-[var(--text-muted)]" v-if="chartState.data.length > 0">
        ...
        <div
            ref="chartContainer"
            class="relative h-80 w-full rounded-lg border border-[var(--border-default)] shadow-inner"
        >
            <!-- Loading state -->
            <div
                v-if="chartState.loading"
                class="absolute inset-0 flex items-center justify-center rounded-lg bg-[var(--surface-overlay)]"
            >
                <div class="flex items-center space-x-2 text-[var(--text-secondary)]">
                    <div class="h-4 w-4 animate-spin rounded-full border-b-2 border-[var(--accent)]"></div>
            ...
            <!-- Error state -->
            <div
                v-if="chartState.error"
                class="absolute inset-0 flex items-center justify-center rounded-lg bg-[var(--surface-overlay)]"
            >
                <div class="text-center text-[var(--text-secondary)]">
                    <div class="mb-2 text-[var(--sell-color)]">⚠️ Error loading chart</div>
                    <div class="mb-4 text-sm">{{ chartState.error }}</div>
                </div>
            </div>

            <!-- Empty state -->
            <div
                v-if="!chartState.loading && !chartState.error && chartState.data.length === 0"
                class="absolute inset-0 flex items-center justify-center rounded-lg bg-[var(--surface-overlay)]"
            >
