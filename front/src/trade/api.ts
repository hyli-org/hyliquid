...
export async function fetchMarketPrice(symbol: string): Promise<{ price: number; change: number; vol: number }> {
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/market/price/${baseAsset}/${quoteAsset}`);
...
export async function fetchInstruments(): Promise<{ instruments: Instrument[]; assets: Asset[] }> {
    ...
    const instruments = await Promise.all(
        data.instruments
            ...
            .map((inst) =>
                fetchMarketPrice(inst.symbol).then((price) =>
                    transformInstrument(inst, price.price, price.change, price.vol),
                ),
            ),
    );
...
export async function fetchOrdersForInstrument(
    ...
    if (!response.ok) {
        throw new Error(
            `Failed to fetch orders for ${baseAssetSymbol}/${quoteAssetSymbol}: ${response.status} ${response.statusText}`,
        );
    }
