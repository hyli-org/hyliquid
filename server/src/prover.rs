...
use hyli_modules::{
    bus::{BusMessage, SharedMessageBus}, log_error, module_bus_client, module_handle_messages, modules::Module
};
    ...
    async fn handle_node_state_event(&mut self, event: NodeStateEvent) -> Result<()> {
        match event {
            NodeStateEvent::NewBlock(block) => {
                ...
                let tx_hashes: Vec<TxHash> = block.parsed_block.txs.iter().map(|(tx_id, _)| tx_id.1.clone()).collect();
                ...
                for (tx_hash, pending_tx) in txs_to_prove {
                    ...
                    tokio::spawn(async move {
                        ...
                        match prover.prove(pending_tx.commitment_metadata, vec![calldata]).await {
                            Ok(proof) => {
