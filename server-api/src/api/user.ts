...
export const userRoutes = (userService: UserService, assetService: AssetService) => {
  return new Elysia({ name: 'user' })
    ...
    .get('/api/user/orders', async ({ auth, query }: { auth: AuthHeaders, query: any }) => {
        ...
        const orders = await userService.getOrdersPaginated(auth.user, pagination);
        return orders;
    })
    .get('/api/user/orders/:baseAssetSymbol/:quoteAssetSymbol', async ({ auth, params, query }: { auth: AuthHeaders, params: { baseAssetSymbol: string, quoteAssetSymbol: string }, query: any }) => {
      ...
      if (!instrumentId) {
        ...
        const orders = await userService.getOrdersByPairPaginated(auth.user, instrumentId, pagination);
        return orders;
    })
